...
var (
	...
	node          corev1.Node
	testDrainer   drain.DrainInterface
	podName       = "test-nginx-pod"
	containerName = "test-nginx-container"
	podNamespace  = "default"
	imageName     = "nginx:latest"
)

func createNewDrainer(config *rest.Config) (drain.DrainInterface, error) {
	...
	vars.Config = config
	return drain.NewDrainer(platform)
}
...
var _ = Describe("Drain Interface", Ordered, func() {
	...
	Context("When node drain and cordon has been requested", func() {
		It("should drain the node and mark it unschedulable", func() {
			By("checking that all nodes are not ready and draining")
			Eventually(func() bool {
				return testPodIsRunning()
			...
			var err error
			testDrainer, err = createNewDrainer(restConfig)
			Expect(err).NotTo(HaveOccurred())
			...
			ctrl.Log.Info("Checking node is cordoned", "nodeName", node.Name)
